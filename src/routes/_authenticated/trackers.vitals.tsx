        ...
        <Dialog open={open} onOpenChange={setOpen}>
          <DialogTrigger asChild>
            <Button className="soma-gradient soma-glow border-0 text-white">
              <Plus className="mr-2 h-4 w-4" /> {t("vitals.logReading", "Log reading")}
            </Button>
          </DialogTrigger>
          <DialogContent>
            <DialogHeader>
              <DialogTitle>{t("vitals.logVital", "Log a vital")}</DialogTitle>
            </DialogHeader>
            <form
              onSubmit={(e) => {
                e.preventDefault();
            ...
            >
              ...
              <div className="grid grid-cols-3 gap-3">
                <div className="col-span-2">
                  <Label>{t("vitals.value", "Value")}</Label>
                  <Input
                    type="number"
                    step="any"
                    required
                ...
                <div>
                  <Label>{t("vitals.unit", "Unit")}</Label>
                  <Input
                    value={form.unit}
        ...
        {abnormalities.length > 0 && (
          <InfoBanner tone="danger">
            <div className="font-semibold text-sm mb-2">
              {t("vitals.healthAlerts", "Health Alerts")}
            </div>
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                {KINDS.filter((k) => k.key !== "weight").map((k) => {
                  ...
                  return (
                    <Card
                      key={k.key}
                      className={abnormality ? "border border-[var(--destructive)]/30" : ""}
                    >
                ...
                <CardContent>
                  {list.isLoading ? (
                            ...
                            >
                              <div>
                                ...
                                <div className="text-xs text-muted-foreground">
                                  {format(new Date(v.taken_at), "PPp")}
                                </div>
                              </div>
                              <div className="text-right">
                                <div
                                  className={`font-display text-xl font-bold ${getVitalStatusColor(abnormality?.status ?? "normal")}`}
                                >
                                  {v.value}
                    ...
                    <EmptyState
                      ...
                      body="Log your first vital to start seeing trends."
                    />
                  )}
                </CardContent>
              </Card>
